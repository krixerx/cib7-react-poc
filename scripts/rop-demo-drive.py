"""Headless end-to-end driver for the two ROP ITS demo services.
...
scenarios. Usage: python scripts/rop-demo-drive.py
"""
...
def scenario1():
    print("=== Scenario 1: ROP Vehicle Registration (happy path) ===")
    bart, homer = token("bart", "bart"), token("homer", "homer")

    pi = api(BASE + "/engine-rest/process-definition/key/ropVehicleRegistration/start",
...
def scenario2():
    ...
    bart, homer = token("bart", "bart"), token("homer", "homer")
    pi = api(BASE + "/engine-rest/process-definition/key/ropLearningPermit/start",
             bart, "POST", {"variables": {}})["id"]
    print("started", pi)
    ...
    wait_for_activity(bart, pi, "Task_RopWaitPermitPayment")
    status = api(BASE + f"/api/public/payments/{pi}/status")
    print("payment page:", status["amount"], status["currency"], "-", status["item"])
    assert status["currency"] == "OMR" and status["amount"] == 6.0, status
    api(BASE + f"/api/public/payments/{pi}/confirm", method="POST", body={})
    print("fee paid")
