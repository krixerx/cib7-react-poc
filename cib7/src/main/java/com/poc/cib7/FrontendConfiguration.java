 ...
 *
 * <p>Same pattern as {@link MailConfiguration} / {@link PdfConfiguration}: a Spring {@code String}
 * bean is auto-injected into the engine's expression manager by bean name, so the BPMN can use
 * {@code ${frontendBaseUrl}} directly with no engine-side wiring.
 *
 * <p>Driven by the {@code FRONTEND_BASE_URL} env var (defaults to {@code http://localhost:5173} for
 * the Vite dev server).
 */
@Configuration
