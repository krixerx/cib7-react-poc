 ...
 *
 * <p>Two endpoints under {@code /api/public/vehicle-registry/**}: a list
 ...
 * to drive the demo:
 * <ul>
 *   <li>{@code vin}, {@code make}, {@code model}, {@code year},
 *       {@code value}, {@code fuelType} so the SPA dropdown reads like a
 *       real listing,</li>
 *   <li>{@code ageYears} computed server-side at request time so the
 *       BPMN doesn't have to compute it in JUEL,</li>
 *   <li>span of fee tiers (&lt;€5k, &lt;€20k, &lt;€50k, &ge;€50k) and
 *       DMN paths (under-age owner / luxury / cheap-old-cheap auto-
 *       approve / default-review) — picking any of the ten will hit a
 *       different combination of branches.</li>
 * </ul>
 */
@RestController
...
public class VehicleRegistryController {
    ...
    public ResponseEntity<VehicleResponse> getVehicle(@PathVariable String vin) {
        int currentYear = Year.now().getValue();
        Optional<Vehicle> match = CATALOG.stream()
                .filter(v -> v.vin().equalsIgnoreCase(vin))
                .findFirst();
        return match
    ...
    /**
     * Hand-curated catalog of believable Estonian-market vehicles. Span is
     * intentional — see the class javadoc for the fee/DMN-tier coverage.
     * VINs are realistic-looking 17-character strings; they don't decode
     * to any real production records.
     */
    private static final List<Vehicle> CATALOG = List.of(
            ...
            new Vehicle("5YJ3E1EA1JF000123", "Tesla",        "Model 3 Long Range",2022, 38000.0, "Electric"),
