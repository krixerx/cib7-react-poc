...
/**
 * Grants the {@code /applicant} and {@code /civil-servant} Keycloak groups
 * the minimum engine authorizations to do their respective jobs across ALL
 * deployed process definitions.
 *
 * <p>Engine authorization is enabled (see {@code application.yaml}), so by
 * default a new group has no permissions and every {@code /engine-rest}
 * call gets either 403'd or — for "is this resource here?" queries —
 * returns a misleading 500 with a "no matching process definition" message,
 * because the engine deliberately hides resources the caller can't read.
 * The {@code /cib7-admin} group is handled by the {@code cibseven-keycloak}
 * plugin's {@code administratorGroupName}; everyone else needs grants here.
 *
 * <p>Resource id is widened to {@code "*"} for the process-definition grants
 ...
 * filters on top, so the wildcard does NOT let a civil servant complete an
 * applicant's task — it only lets them see and work on tasks owned by a
 * candidate group they belong to.
 *
...
public class AuthorizationBootstrap {
    ...
    /**
     * Applicants start, read, and complete their own process instances.
     * Per-task assignee filters in the BPMN keep them out of civil-servant
     * tasks even with the wildcard resource id.
     */
    private void grantApplicant(AuthorizationService auth) {
        grant(auth, APPLICANT_GROUP, Resources.PROCESS_DEFINITION, "*",
    ...
    /**
     * Civil servants don't START processes (applicants do), but they need to
     * see every deployed definition (so the worklist's left-pane filter and
     * the read-only history view can resolve form keys from BPMN XML), see
     * every instance and its history, see every task, and complete the tasks
     * that route to the civil-servant candidate group via the BPMN. They
     * also need to retry failed jobs from the worklist's incident pane —
     * UPDATE_INSTANCE on the process definition covers that.
     */
    private void grantCivilServant(AuthorizationService auth) {
        grant(auth, CIVIL_SERVANT_GROUP, Resources.PROCESS_DEFINITION, "*",
                ProcessDefinitionPermissions.READ,
                ProcessDefinitionPermissions.READ_INSTANCE,
                ProcessDefinitionPermissions.READ_HISTORY,
                ProcessDefinitionPermissions.UPDATE_INSTANCE,
    ...
    private void grant(AuthorizationService auth, String group, Resource resource,
        ...
        LOG.info("Granted {} on {}:{} to group {}",
                permissions, resource.resourceName(), resourceId, group);
    }
}
