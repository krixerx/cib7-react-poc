...
/**
 * Redirects the user to Keycloak's OIDC logout endpoint when they sign out of
 * a webapp, so the SSO session ends everywhere — not just locally.
 *
 ...
 * authorization endpoint is rewritten to the logout endpoint by string
 * replacement, and the current {@code id_token} is passed as
 * {@code id_token_hint} so Keycloak can identify the session.
 */
@Service
public class KeycloakLogoutHandler implements LogoutSuccessHandler {
    ...
    public void onLogoutSuccess(HttpServletRequest request, HttpServletResponse response,
        ...
        String logoutUrl = oauth2UserLogoutUri
