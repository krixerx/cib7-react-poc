...
/**
 * Spring Security wiring for the legacy CIB seven webapps (Cockpit /
 * Tasklist / Admin), served from {@code /camunda/**}.
 *
 * <p>Verbatim from the cibseven-keycloak plugin's {@code sso-kubernetes}
 * example, repackaged under {@code com.poc.cib7.keycloak.webapp}. The
 * filter chain matches webapp paths only (plus OAuth2's
 * {@code /oauth2/authorization/**}, {@code /login/**}, {@code /logout}) and
 * uses Authorization Code login against Keycloak. The
 * {@link ContainerBasedAuthenticationFilter}, registered as a servlet filter
 * at order 201, runs after Spring Security has populated the principal and
 * hands the user + groups to the engine's IdentityService via
 * {@link KeycloakAuthenticationProvider}.
 *
 * <p>The {@code @Order(2)} on this chain places it after the
 * {@code @Order(1)} chain for {@code /engine-rest/**} in
 * {@link com.poc.cib7.keycloak.RestApiSecurityConfig}, so REST requests keep
        ...
        return http
                .securityMatcher(request -> {
                    ...
                    return path.startsWith(legacyWebappPath)
                            || path.startsWith(OAuth2AuthorizationRequestRedirectFilter.DEFAULT_AUTHORIZATION_REQUEST_BASE_URI)
