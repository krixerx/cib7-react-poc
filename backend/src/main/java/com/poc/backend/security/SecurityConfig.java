 ...
 *       credential.
 *   <li><b>Internal</b> (@Order(1)) — the two engine→backend endpoints ({@code /move-pending},
 *       {@code /server-upload}) that BPMN service tasks call via the http-connector. Auth is the
 *       shared {@code X-Internal-Token} header; no JWT because the caller is the engine, not a
 *       logged-in user.
 *   <li><b>JWT</b> (@Order(2)) — everything else under {@code /api/documents/**}, called by the SPA
 *       with the user's Keycloak Bearer. Validation (signature via the internal JWKS URL, issuer
 *       string-compare against the public URL, {@code cib7-rest-api} audience) is configured
 *       entirely through {@code spring.security.oauth2.resourceserver.jwt.*}.
 * </ol>
 *
...
public class SecurityConfig {
  ...
  public SecurityFilterChain internalApiSecurity(HttpSecurity http) throws Exception {
    return http.securityMatcher(INTERNAL_MOVE_PENDING, INTERNAL_SERVER_UPLOAD)
        .csrf(csrf -> csrf.disable())
        .authorizeHttpRequests(authorize -> authorize.anyRequest().permitAll())
        .addFilterBefore(
  ...
  public SecurityFilterChain documentsJwtSecurity(HttpSecurity http) throws Exception {
    return http.securityMatcher("/api/documents/**")
        .csrf(csrf -> csrf.disable())
