...
/**
 * Plate allocation for {@code rop-vehicle-registration.bpmn} — called by {@code
 * Task_RopAllocatePlate} strictly after the fee payment is correlated. Persists the registration
 * record (the demo's "integration to database" requirement) and returns the plate: a generated
 * Omani-style {@code NNNNN AB} number for {@code plateOption=random}, or the caller's reserved
 * number verbatim ("previously reserved number plates" in the demo's service path).
 */
@RestController
@RequestMapping("/api/public/rop/plates")
public class RopPlateController {

  /** Letters used on Omani plates (Latin series, simplified for the POC). */
  private static final String PLATE_LETTERS = "ABDHMRSTWY";

  private final PlateRegistrationRepository plates;

  public RopPlateController(PlateRegistrationRepository plates) {
    this.plates = plates;
  }
