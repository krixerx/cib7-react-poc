 ...
 *
 * <p>Mirrors {@link com.poc.backend.owner.OwnerConfirmationController} line-for-line; OÜ founder
 * semantics swapped for vehicle-owner semantics. Both controllers use the same pattern:
 * per-participant UUID token in the URL is the credential, message correlation on a local variable
 * carries the decision back into the multi-instance subprocess.
 *
 ...
 * </pre>
 *
 * <p>Token-to-process lookup is intentionally O(active instances): scans active {@code
 * businessRegistration} instances and matches against {@code applicantToken} / {@code
 * additionalFounders[].token}. Move to a dedicated index table if traffic grows. Two controllers
 * doing this same scan is the kind of duplication that earns its own abstraction at N=3 — see
 * redomain plan PR #3 notes.
 */
@RestController
...
public class FounderSignatureController {
  ...
  public ResponseEntity<?> getStatus(@PathVariable String token) {
    ...
    if (lookup == null) {
      return ResponseEntity.status(HttpStatus.NOT_FOUND)
          .body(new ErrorResponse("unknown_token", "This signing link is unknown or has expired."));
    }
  ...
  public ResponseEntity<?> sign(@PathVariable String token, @RequestBody SignRequest req) {
    ...
    if (signatures.has(token)) {
      return ResponseEntity.status(HttpStatus.CONFLICT)
          .body(new ErrorResponse("already_signed", "You have already signed this registration."));
    }
    ...
    if (!correlated) {
      return ResponseEntity.status(HttpStatus.CONFLICT)
          .body(
              new ErrorResponse(
                  "not_waiting", "This case is no longer waiting for co-founder signatures."));
    }
  ...
  public ResponseEntity<?> submitToRegister(@PathVariable String token) {
    ...
    if (!"ready_to_send".equals(status.state())) {
      return ResponseEntity.status(HttpStatus.CONFLICT)
          .body(new ErrorResponse("not_ready", "Not all co-founders have signed yet."));
    }
    ...
    if (!correlated) {
      return ResponseEntity.status(HttpStatus.CONFLICT)
          .body(
              new ErrorResponse(
                  "not_waiting", "This case is not waiting on a submit-to-register signal."));
    }
   ...
   * applicantToken} matches, or which has a matching entry in {@code additionalFounders}. Linear is
  ...
  private void writeSignature(ObjectNode signatures, String token, String decision, String reason) {
  ...
  private FounderStatus buildStatus(TokenLookup lookup, String requestToken) {
    ...
    String applicantName =
        (Objects.toString(vars.applicantFirstName(), "")
                + " "
                + Objects.toString(vars.applicantLastName(), ""))
            .trim();
    ...
    FounderEntry current =
        founders.stream()
            .filter(f -> Objects.equals(f.token(), requestToken))
            .findFirst()
            .orElse(null);

    FounderEntry rejecter =
        founders.stream().filter(f -> "rejected".equals(f.status())).findFirst().orElse(null);
  ...
  private static FounderEntry buildFounderEntry(
      String name, String email, String token, boolean isApplicant, JsonNode signatures) {
