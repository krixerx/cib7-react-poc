...
/**
 * Public REST surface for the owner-confirmation flow in {@code vehicle-registration.bpmn}.
 *
 * <p>Endpoints under {@code /api/public/**} are unauthenticated (see {@link
 * com.poc.backend.security.SecurityConfig}). Authorization is by knowledge of the per-owner UUID
 * token: the applicant's form generates a token for the applicant and one for each additional
 * owner, the engine stores them as process variables, and the controller correlates back to the
 * waiting receive task in the multi-instance subprocess via the message API's {@code
 * localCorrelationKeys}.
 *
 * <p>All engine access goes through {@link EngineClient} over {@code /engine-rest} — this service
 * has no embedded engine. Spin JSON process variables round-trip as engine type {@code Json} and
 * are handled here as Jackson trees.
 *
 ...
 * </pre>
 *
 * <p>Token-to-process lookup is intentionally O(active instances): the POC scans active {@code
 * vehicleRegistration} instances and matches against {@code applicantToken} / {@code
 * additionalOwners[].token}. Move to a dedicated index table if the case load ever grows beyond
 * demo scale.
 */
@RestController
...
public class OwnerConfirmationController {
  ...
  public ResponseEntity<?> getStatus(@PathVariable String token) {
    ...
    if (lookup == null) {
      return ResponseEntity.status(HttpStatus.NOT_FOUND)
          .body(
              new ErrorResponse(
                  "unknown_token", "This confirmation link is unknown or has expired."));
    }
  ...
  public ResponseEntity<?> confirm(@PathVariable String token, @RequestBody ConfirmRequest req) {
    ...
    if (Boolean.TRUE.equals(vars.rejectedByOwner())) {
      return ResponseEntity.status(HttpStatus.CONFLICT)
          .body(
              new ErrorResponse(
                  "already_rejected", "Another owner has already rejected this application."));
    }
    if (Boolean.TRUE.equals(vars.sentToProcess())) {
      return ResponseEntity.status(HttpStatus.CONFLICT)
          .body(
              new ErrorResponse(
                  "already_sent", "This case has already been sent to the back office."));
    }
    ObjectNode confirmations = vars.ownerConfirmations();
    if (confirmations.has(token)) {
      return ResponseEntity.status(HttpStatus.CONFLICT)
          .body(new ErrorResponse("already_signed", "You have already signed this application."));
    }
    ...
    boolean correlated =
        engine.correlateMessage(
            "OwnerConfirmation", lookup.processInstanceId(), Map.of("ownerToken", token), Map.of());
    if (!correlated) {
      return ResponseEntity.status(HttpStatus.CONFLICT)
          .body(
              new ErrorResponse(
                  "not_waiting", "This case is no longer waiting for owner signatures."));
    }
  ...
  public ResponseEntity<?> sendToProcess(@PathVariable String token) {
    ...
    if (!"ready_to_send".equals(status.state())) {
      return ResponseEntity.status(HttpStatus.CONFLICT)
          .body(new ErrorResponse("not_ready", "Not all owners have signed yet."));
    }

    boolean correlated =
        engine.correlateMessage(
            "SendToProcess", lookup.processInstanceId(), Map.of(), Map.of("sentToProcess", true));
    if (!correlated) {
      return ResponseEntity.status(HttpStatus.CONFLICT)
          .body(
              new ErrorResponse(
                  "not_waiting", "This case is not waiting on a send-to-process signal."));
    }
  ...
  /**
   * Linear scan over active {@value #PROCESS_KEY} instances. Returns the one whose {@code
   * applicantToken} matches, or which has a matching entry in {@code additionalOwners}. Linear is
   * fine for a POC; add an index table if traffic grows.
   */
  ...
  private void writeConfirmation(
      ObjectNode confirmations, String token, String decision, String reason) {
  ...
  private OwnerStatus buildStatus(TokenLookup lookup, String requestToken) {
    ...
    String applicantName =
        (Objects.toString(vars.firstName(), "") + " " + Objects.toString(vars.lastName(), ""))
            .trim();
    ...
    OwnerEntry rejecter =
        owners.stream().filter(o -> "rejected".equals(o.status())).findFirst().orElse(null);
  ...
  private static OwnerEntry buildOwnerEntry(
      String name, String email, String token, boolean isApplicant, JsonNode confirmations) {
