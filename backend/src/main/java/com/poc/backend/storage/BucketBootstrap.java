 ...
 *
 * <ol>
 *   <li>Create the bucket if it doesn't exist.
 *   <li>Apply a CORS policy so the browser's preflight on direct PUT / GET succeeds.
 *   <li>Apply a lifecycle rule that expires {@code pending/} keys after 1 day — handles abandoned
 *       uploads where the applicant picked a file but never submitted the form.
 * </ol>
 *
 * <p>Any failure is logged loudly but does NOT crash the application — RustFS may be temporarily
 * unhealthy and the BPMN tasks would surface a more useful error than a startup crash.
 */
@Component
public class BucketBootstrap {
  ...
  public void bootstrap() {
    try {
      ...
      applyLifecycle();
      LOG.info(
          "RustFS bucket '{}' ready (CORS allowed origin: {})",
          props.getBucket(),
          props.getCorsAllowedOrigin());
    } catch (Exception e) {
      LOG.error("Failed to bootstrap RustFS bucket '{}': {}", props.getBucket(), e.getMessage(), e);
    }
  }
    ...
    s3.putBucketLifecycleConfiguration(
        PutBucketLifecycleConfigurationRequest.builder()
            ...
            .lifecycleConfiguration(
                BucketLifecycleConfiguration.builder().rules(List.of(pendingExpiry)).build())
            .build());
  }
}
