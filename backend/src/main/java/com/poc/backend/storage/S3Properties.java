...
/**
 * Bound from the {@code app.s3.*} block in application.yaml. Holds the
 * two URLs (internal docker network vs browser-visible), the bucket
 * name, the access/secret pair, and the byte cap that downstream code
 * enforces as a signed PUT header.
 */
