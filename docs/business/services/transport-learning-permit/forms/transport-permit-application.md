# Form: `transport-permit-application`

**Form id:** `transport-permit-application` (kebab-case, globally unique)
**BPMN task:** `Task_TransportPermitApplication`
**Audience:** `initiator`
**Mode:** `entry`

## Intro

"Apply for a driving learning license from the Transport Authority
(Transport Authority). **Service fee: 6 EUR** — payable after your
...
| `residencyStatus` | `Residency status` | `select` (`citizen` Citizen \| `gcc-citizen` GCC-country citizen \| `resident` Resident) | yes | `data.residencyStatus` (default `citizen`) | one of the three values |
...
- The eye test is NOT a form field — the demo's terms require the result
  ...
  `transport-driver-clearance` service task fetches by `civilId` right after
  submission.
