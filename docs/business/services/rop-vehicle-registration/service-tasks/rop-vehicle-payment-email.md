# Service task: `rop-vehicle-payment-email`

**BPMN task:** `Task_RopVehiclePaymentEmail`
...
`Task_RopWaitFeePayment` until the fee is confirmed on the public payment
page.
...
`cib7/src/main/resources/templates/rop-vehicle-payment-email.json.ftl`.
...
`${registrationFee} OMR` (use the defensive numeric-coercion pattern from
the FreeMarker-numeric-vars memory before `?string("0.000")`; OMR has 3
...
- The shared `PaymentController` resolves this definition key
  (`ropVehicleRegistration`) to the `registrationFee` variable and renders
  the pay page in OMR — the email's amount and the pay page's amount come
