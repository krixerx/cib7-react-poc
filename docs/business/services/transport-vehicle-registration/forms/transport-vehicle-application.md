# Form: `transport-vehicle-application`

**Form id:** `transport-vehicle-application` (kebab-case, globally unique)
**BPMN task:** `Task_TransportVehicleApplication`
**Audience:** `initiator`
**Mode:** `entry` (collects new data; supports send-back resubmit too)

## Intro

First-submit: "Register a new vehicle with the Transport Authority
(Transport Authority). The registration fee for your vehicle category is
...
The form renders a highlighted **"Registration fee: X EUR"** line that
updates live as the user changes `vehicleCategory`. The client-side table
mirrors the `transport-vehicle-fee` DMN (the DMN stays authoritative):

| `vehicleCategory` value | UI label | Fee (EUR) |
|---|---|---|
...
(Fee schedule from the demo document, "Service Fee" table.)
...
| `residencyStatus` | `Residency status` | `select` (`citizen` Citizen \| `resident` Resident \| `diplomat` Diplomatic mission \| `visitor` Visitor) | yes | `data.residencyStatus` (default `citizen`) | one of the four values |
...
rejection branches (`DEMOFAILINSP01` — failed inspection,
`DEMONOINSURE02` — uninsured, `DEMOFINESDUE03` — outstanding fines).
...
- Civil-ID checksum and the real local plate taxonomy are out of POC
