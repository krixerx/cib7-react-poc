# Service task: `transport-allocate-plate`

**BPMN task:** `Task_TransportAllocatePlate`
...
`transport_plate_registrations`, JPA entity `PlateRegistration`) and returns the
allocated plate — random generic `NNNNN AB` for `plateOption=random`,
...
`cib7/src/main/resources/templates/transport-allocate-plate.json.ftl`.
