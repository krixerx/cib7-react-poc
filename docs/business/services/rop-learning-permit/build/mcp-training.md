# ropLearningPermit — guidance for the LLM

An applicant requests a driving learning license from the General Traffic
Department of the Royal Oman Police (ITS Demo Scenario 2). The service
fee is a flat 6 OMR, payable after approval. The system fetches the
...
To start a ropLearningPermit, you need these pieces of information:

- **applicantName** — full name as on the civil ID.
- **applicantEmail** — where all notifications are sent.
- **civilId** — Omani civil number, exactly 8 digits (drives the
...
- Process `running`, "Wait for fee payment" active → approved; the
  applicant must pay 6 OMR at `/pay/{processInstanceId}`.
...
- *"How much does it cost?"* — flat 6 OMR, paid after approval.
