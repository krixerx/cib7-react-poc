...
- **MCP sidecar** — a standalone Node + TypeScript microservice that
  ...
  Desktop (or any MCP-capable client) connects via `/mcp/sse`, completes
  OAuth2 PKCE-loopback against Keycloak, and drives the deployment through
  eight tools (`list_services`, `describe_service`, `start_process`,
  `list_my_tasks`, `get_form_schema`, `complete_task`, `list_my_processes`,
  `query_user_history`). The sidecar is a **stateless Bearer-proxy** in
  front of `/engine-rest` — it never validates JWTs locally, never holds
  refresh tokens, and never persists user state. Per-service variable
  schemas come from `docs/business/services/<svc>/build/mcp-service.json`
  generated by `/service-builder`. Full module guide:
  [`mcp.md`](mcp.md).
...
| MCP sidecar | `mcp/` (Node + TypeScript + Express + `@modelcontextprotocol/sdk`) | local module, compose service | Streamable HTTP MCP transport at `/mcp/sse`; OAuth2 PKCE-loopback against Keycloak; Bearer-forwards to `/engine-rest`; Ajv-validates inputs against per-service manifests |
...
| Container orchestration | Docker Compose | `docker-compose.yml` | Seven services: `keycloak`, `cib7`, `frontend`, `mailpit`, `gotenberg`, `pdf-renderer`, `mcp` |
...
- **mcp** — built from `mcp/Dockerfile` with the repo root as build
  ...
  TypeScript + Express + `@modelcontextprotocol/sdk`. Internal only on
  port 8090; exposed publicly via nginx at `/mcp/sse` and the
  OAuth resource metadata at `/.well-known/oauth-protected-resource`.
  Depends on `keycloak` (healthy) and `cib7` (started). Env-driven
  (`MCP_RESOURCE_URL`, `KEYCLOAK_ISSUER_URL`, `ENGINE_URL`,
  `SERVICES_SPEC_DIR`) so the same image works in dev and CI without
  rebuilds.
