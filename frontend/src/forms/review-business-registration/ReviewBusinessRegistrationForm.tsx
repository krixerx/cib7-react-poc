...
/**
 * Civil-servant review form for businessRegistration (PartB). Shows the
 * applicant's submitted data read-only with two actions:
 *
 ...
 *                decision="sendback" + sendBackReason so the applicant sees
 *                why the case was returned. The process loops back to the
 *                applicant's business-details task.
 *
  ...
  return (
    <div className="form">
      <p className="form-intro">
        {readOnly
          ? 'A read-only view of the submitted business registration and the reviewer’s decision.'
          : 'Review the proposed business registration. Approve to register the company; send back with a reason to ask the applicant for corrections.'}
      </p>
      ...
      {!readOnly && showSendBack && (
        <label className="field">
          ...
          <textarea
            ...
            placeholder="Tell the applicant what to fix (e.g. share capital below €2500)."
            autoFocus
          />
        </label>
      )}
