...
/**
 * Traffic officer review form for transportVehicleRegistration (PartB) — Transport Authority
 ...
 * docs/business/services/transport-vehicle-registration/forms/transport-vehicle-officer-review.md
 */
...
export default function TransportVehicleOfficerReviewForm({
  data,
  onComplete,
  submitting,
  readOnly,
}: FormProps) {
  const { t } = useTranslation('transport-vehicle-officer-review');
  ...
  const fee =
    data.registrationFee != null && data.registrationFee !== ''
      ? formatCurrency(Number(String(data.registrationFee).replace(/[^0-9.]/g, '')), 'EUR')
      : '—';
